...
class Home extends Component {
    ...
    componentDidMount() {
            ...
            xhr.onreadystatechange = () => {
                ...
                // console.log("xhr.status "+xhr.status);
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        ...
                        console.log("xhr.responseText.data "+JSON.parse(xhr.responseText).username);

                        this.setState({
                            //profileImage
                            //Profile picture obtained from the API is stored in profile_picture & username in userName
                            profile_picture: JSON.parse(xhr.responseText).profile_picture,
                            userName: JSON.parse(xhr.responseText).username
                        });
                    }
            };
            console.log(this.props.baseUrl + "17841419495779319?fields=id,username,profile_picture,media_count&access_token=" + that.state.accessToken);
            xhr.open("GET", this.props.baseUrl + "17841419495779319?fields=id,username,profile_picture,media_count&access_token=" + that.state.accessToken);
            xhr.send();
        }
        ...
        if (this.state.isLoggedIn) {
            ...
            xhrImages.onreadystatechange = () => {
                    if (xhrImages.readyState === 4 && xhrImages.status === 200) {
                        console.log("xhrImages.responseText "+xhrImages.responseText);
                        console.log("xhrImages.responseText.data.caption "+JSON.parse(xhrImages.responseText).data.caption);

                        var imageArr = JSON.parse(xhrImages.responseText).data;
            ...
            console.log(this.props.baseUrl + "me/media?fields=id,caption&access_token=" + that.state.accessToken);

            xhrImages.open("GET", this.props.baseUrl + "me/media?fields=id,caption&access_token=" + that.state.accessToken);
            xhrImages.send();
        }
    }
